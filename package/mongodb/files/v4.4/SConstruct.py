...
def add_option(name, **kwargs):
    ...
    if 'metavar' not in kwargs and kwargs.get('type', None) == 'choice':
        kwargs['metavar'] = '[' + '|'.join('%s' %choice for choice in kwargs['choices']) + ']'
...
if not detectConf.CheckForToolchain('GCC', "C++", "CXX", ".cpp"):
    env.ConfError("Couldn't find GCC compiler")
...
can_use_libunwind = (env['TARGET_ARCH'] == 'x86_64')
...
def doConfigure(myenv):
    ...
    conf.env.AppendUnique(CXXFLAGS=['-Wno-overloaded-virtual'])
    conf.env.Append(
        CPPDEFINES=[
    ...
    conf.FindSysLibDep("boost_thread",["boost_thread"],language='C++');
    
    env.Append(LIBS=['boost_chrono','boost_date_time','boost_filesystem','boost_regex','boost_thread','boost_atomic'])
    conf.FindSysLibDep("boost_log",["boost_log"],language='C++');
    env.Append(LIBS=[])
...
should_dagger = "dagger" in COMMAND_LINE_TARGETS
